...
class PostItBall(tk.Toplevel):
    def __init__(self, parent, text, color, manager, start_time=None):
        ...
        self.btn_done = tk.Button(self.canvas, text="✕", command=self.close_note, font=("Arial", 8, "bold"), bg="#ffcccc", relief="flat")
...
class AppManager:
    def __init__(self):
        ...
        tk.Button(self.root, text="Lançar Bolinha", command=lambda: self.add_note(), bg="#ccffcc").pack(pady=10)

        tk.Label(self.root, text="Bolinhas Ativas:", font=("Arial", 10, "bold")).pack(pady=5)
    ...
    def save_data(self):
        data = [{"text": n.text, "color": n.color, "start_time": n.start_time} for n in self.notes]
            ...
            try:
                with open(SAVE_FILE, 'r', encoding='utf-8') as f:
                    for item in json.load(f):
                        self.add_note(item['text'], item['color'], item['start_time'], save=False)
            except: pass

    def add_note(self, text=None, color=None, start_time=None, save=True):
        if text is None:
            text = self.entry.get() or "Nota"
            self.entry.delete(0, tk.END)
        new_note = PostItBall(self.root, text, color if color else self.selected_color, self, start_time)
    ...
    def update_list(self):
        ...
        for note in self.notes:
            ...
            tk.Label(frame, text=note.text[:15], bg=note.color).pack(side="left", padx=5)
            tk.Button(frame, text="Remover", command=note.close_note).pack(side="right")
